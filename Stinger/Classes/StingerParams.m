...
- (NSValue *)invokeOriginal {
  ...
  if (signature.methodReturnLength) {
    void *retLoc = NULL;
    retLoc = malloc(signature.methodReturnLength);
    [originalInvocation getReturnValue:retLoc];
    // 使用NSData管理retLoc内存
    NSData *data = [NSData dataWithBytesNoCopy:retLoc
                                        length:signature.methodReturnLength
                                  freeWhenDone:YES];
    // 使用NSValue包装内存
    NSValue *retValue = [NSValue valueWithBytes:data.bytes
                                       objCType:signature.methodReturnType];
    // data关联到retValue, 保证在retValue释放同时释放data及其管理的retLoc内存
    static const char retValueKey;
    objc_setAssociatedObject(retValue, &retValueKey, data, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (NSValue*)invokeOriginal:(void*)useless, ... {
...
#undef case_size
  ...
  if (sig.methodReturnLength) {
    void *retLoc = NULL;
    retLoc = malloc(sig.methodReturnLength);
    [inv getReturnValue:retLoc];
    // 使用NSData管理retLoc内存
    NSData *data = [NSData dataWithBytesNoCopy:retLoc
                                        length:sig.methodReturnLength
                                  freeWhenDone:YES];
    // 使用NSValue包装内存
    NSValue *retValue = [NSValue valueWithBytes:data.bytes
